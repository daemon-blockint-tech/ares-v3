...
impl Default for PolicyEngine {
    fn default() -> Self {
        ...
        capabilities.insert("fs.read".to_string(), CapabilityLevel {
            action: "fs.read".to_string(),
            level: PermissionLevel::Allow,
            requires_approval: false,
            description: "Read files within project directory".to_string(),
        });
        capabilities.insert("fs.write".to_string(), CapabilityLevel {
        ...
        capabilities.insert("fs.delete".to_string(), CapabilityLevel {
        ...
        capabilities.insert("network.fetch".to_string(), CapabilityLevel {
            action: "network.fetch".to_string(),
            level: PermissionLevel::Allow,
            requires_approval: false,
            description: "Fetch web content from allowed domains".to_string(),
        });
        capabilities.insert("network.scan_remote".to_string(), CapabilityLevel {
        ...
        // Solana capabilities
        capabilities.insert("solana.local_test".to_string(), CapabilityLevel {
            action: "solana.local_test".to_string(),
            level: PermissionLevel::Allow,
            requires_approval: false,
            description: "Run tests on local validator".to_string(),
        });
        capabilities.insert("solana.mainnet_fork".to_string(), CapabilityLevel {
            action: "solana.mainnet_fork".to_string(),
            level: PermissionLevel::Escalate,
            requires_approval: true,
            description: "Fork mainnet for simulation — REQUIRES APPROVAL".to_string(),
        });
        capabilities.insert("solana.mainnet_deploy".to_string(), CapabilityLevel {
        ...
        capabilities.insert("exploit.local_poc".to_string(), CapabilityLevel {
            action: "exploit.local_poc".to_string(),
            level: PermissionLevel::Allow,
            requires_approval: false,
            description: "Generate and run PoC in sandbox environment".to_string(),
        });
        capabilities.insert("exploit.mainnet_execute".to_string(), CapabilityLevel {
            action: "exploit.mainnet_execute".to_string(),
            level: PermissionLevel::Deny,
            requires_approval: true,
            description: "Execute exploit on mainnet — NEVER ALLOWED".to_string(),
        });
        capabilities.insert("exploit.third_party_scan".to_string(), CapabilityLevel {
...
impl PolicyEngine {
        ...
        if let Some(path) = policy_file {
            if path.exists() {
                ...
                let content = std::fs::read_to_string(path)
                    .map_err(|e| AresError::PolicyViolation(format!("Failed to read policy: {}", e)))?;
                let engine: PolicyEngine = toml::from_str(&content)
                    .map_err(|e| AresError::PolicyViolation(format!("Failed to parse policy: {}", e)))?;
                info!("Policy loaded: {} v{}", engine.constitution.name, engine.constitution.version);
                return Ok(engine);
            }
        }
    ...
    pub fn check_scan_permission(&self, target_path: &Path) -> AresResult<()> {
        ...
        let allowed = self.sandbox_boundary.allowed_read_paths.iter().any(|p| {
            target_str.starts_with(&p.to_string_lossy().to_string())
        });
        
        if !allowed {
            // Check if it's explicitly blocked
            let blocked = self.sandbox_boundary.blocked_paths.iter().any(|p| {
                target_str.contains(&p.to_string_lossy().to_string())
            });
            
            if blocked {
                error!("POLICY VIOLATION: Scanning blocked path: {}", target_str);
                return Err(AresError::PolicyViolation(
                    format!("Scanning blocked path: {}", target_str)
                ));
            }
        }
    ...
    pub fn check_capability(&self, action: &str) -> AresResult<PermissionLevel> {
        match self.capabilities.get(action) {
            Some(cap) => {
                match cap.level {
                    ...
                    PermissionLevel::Escalate => {
                        warn!("Capability '{}' requires escalation: {}", action, cap.description);
                        Ok(PermissionLevel::Escalate)
                    }
                    PermissionLevel::Deny => {
                        error!("Capability '{}' denied: {}", action, cap.description);
                        Err(AresError::PolicyViolation(
                            format!("Action '{}' is denied by policy: {}", action, cap.description)
                        ))
                    }
                }
            }
            None => {
                ...
                Err(AresError::PolicyViolation(
                    format!("Unknown capability '{}': default deny", action)
                ))
            }
        }
    }
