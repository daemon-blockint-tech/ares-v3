...
pub async fn execute(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        if name_str.starts_with("ares-report-") && name_str.ends_with(".json") {
            ...
            let report: ares_core::AuditReport = serde_json::from_str(&content)
                .map_err(|e| ares_core::AresError::Parse(format!("Failed to parse report: {}", e)))?;
            ...
            match format {
                crate::ReportFormat::Json => {
                    let out = output_path.as_ref().unwrap_or(&scan_output.join(format!("{}-report.json", report.target.name))).clone();
                ...
                crate::ReportFormat::Markdown => {
                    ...
                    let out = output_path.as_ref().unwrap_or(&scan_output.join(format!("{}-report.md", report.target.name))).clone();
                    tokio::fs::write(&out, md).await?;
                    info!("Markdown report written to: {:?}", out);
                }
                crate::ReportFormat::Html => {
                    let html = generate_html_report(&report);
                    let out = output_path.as_ref().unwrap_or(&scan_output.join(format!("{}-report.html", report.target.name))).clone();
                    tokio::fs::write(&out, html).await?;
                    info!("HTML report written to: {:?}", out);
                }
                crate::ReportFormat::Pdf => {
                    let out = output_path.as_ref().unwrap_or(&scan_output.join(format!("{}-report.pdf", report.target.name))).clone();
                    crate::commands::pdf::generate_scan_pdf(&report, &out)
                        .map_err(|e| ares_core::AresError::Execution(format!("PDF generation failed: {}", e)))?;
                    info!("PDF report written to: {:?}", out);
                }
                crate::ReportFormat::GithubIssue => {
                    let issue = generate_github_issue(&report);
                    let out = output_path.as_ref().unwrap_or(&scan_output.join(format!("{}-github-issue.md", report.target.name))).clone();
    ...
    if !found {
        ...
        return Err(ares_core::AresError::NotFound("No scan results found".to_string()));
    }
